...
Use KVM, IPMI or other ways to accomplish Stonith
"""
import time
import socket
...
class KVMStonithHost(StonithHost):
    """
    General class of stonith for each host
    """
    def __init__(self, server_host, guest_host, domain_name=None):
        super(KVMStonithHost, self).__init__(guest_host)
        # Type: ssh_host.SSHHost
        self.ksh_server_host = server_host
        if domain_name is None:
            # The KVM domain name seen in libvirt
            self.ksh_domain_name = self.sth_host.sh_hostname
        else:
            self.ksh_domain_name = domain_name
    ...
    def sth_power_status(self, log):
        ...
        it or ssh to it.
        """
        server_host = self.ksh_server_host
    ...
    def sth_start(self, log):
        ...
        log.cl_info("starting host [%s] using \"virsh start\"",
                    guest_hostname)
        command = ("virsh start %s" % (self.ksh_domain_name))
    ...
    def sth_raw_stop(self, log):
        ...
        command = ("virsh destroy %s" % (self.ksh_domain_name))
        retval = server_host.sh_run(log, command)
