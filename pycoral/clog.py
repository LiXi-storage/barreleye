...
def console_should_log(record):
    """
    If a record starts with STDOUT_KEY, do not print to console since it
    should already been printed
    """
    if record.msg.startswith(STDOUT_KEY):
        return False
    return True
...
def colorless_message(message):
    ...
    for color_seq in COLOR_SEQS:
        message = message.replace(color_seq, "")
    return message
...
class CoralLog():
    ...
    def cl_config(self, console_level=logging.INFO):
        ...
        if self.cl_stderr_color is None:
            self.cl_stderr_color = sys.stderr.isatty()
        colorful = self.cl_stderr_color
        if self.cl_stdout_color is None:
            self.cl_stdout_color = sys.stdout.isatty()
        ...
        if name is None:
            ...
            console_filter.filter = console_should_log
    ...
    def _cl_log_raw(self, level, message):
        """
        Emit the log.
        """
        try:
